...
public:
    explicit Fishhook(QWidget *parent = nullptr) : QWidget(parent)
    {
        ...
        background.load(":/images/beachbg.png");
    }
...
protected:
    void paintEvent(QPaintEvent *) override // This will display a blue background underneath the beach bg Arantxa drew, also accounting for if the image is null
    {
        ...
        if (!background.isNull())
            painter.drawPixmap(rect(), background); // This accounts for if the background is a null image, will scale to the window
        else
        ...
        painter.drawLine(300, 28, 300, hook_Y);
...
{
public:
    Fish(QPoint pos) : Sealife(":/images/Fish.png", pos, QSize(60, 40), -10, 0)
    {
    }
};
...
{
public:
    Jellyfish(QPoint pos) : Sealife(":/images/Jellyfish.png", pos, QSize(100, 60), 6, 0)
    {
    }
};
...
{
public:
    Sharks(QPoint pos) : Sealife(":/images/Shark.png", pos, QSize(400, 100), -5, 0)
    {
    }
};
...
{
public:
    OscarFish(QPoint pos) : Sealife(":/images/OscarFish.png", pos, QSize(800, 100), -1, 0)
    {
    }
};
...
private:
    ...
    {
        timer.stop();
        QMessageBox::information(this, "Game Over", "You reached 20 points! (In memory of Oscar The Fish)");
    ...
    void checkCollisions() //Function made to account for when the uses connect the fishing pole with a fish or enemy
    {
        ...
        for (auto c: creatures)
        {
            if (c->getRect().intersects(hookRect))
            {
                ...
                if (score >= 20) //One score reaches 20 points, it will call the game over function
                {
                    gameOver();
                    return;
                }
...
int main(int argc, char *argv[])
{
    ...
    ocean.setWindowTitle("Go Fishing with StarScream"); //Shows the title screen
    ocean.show();
